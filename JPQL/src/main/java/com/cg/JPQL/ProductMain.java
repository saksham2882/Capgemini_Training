package com.cg.JPQL;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Data;

import java.time.LocalDate;
import java.util.List;


public class ProductMain {
    public static void main(String[] args) {
        EntityManagerFactory emf= Persistence.createEntityManagerFactory("my-pu");
        EntityManager em= emf.createEntityManager();

        // ------- 1. Fetching  all details --------
//        TypedQuery<Product> q=em.createQuery("select p from Product p", Product.class);
//        List<Product> li=q.getResultList();
//        li.forEach(p -> System.out.println(p));


        // ------- 2. Fetching some particular details --------
//        Query q = em.createQuery("select p.name,p.price,p.mfd from Product p");
//        List<Object[]> li = q.getResultList();
//        for(Object[] obj : li) {
//            System.out.println(obj[0] + " " + obj[1] + " " + obj[2]);
//        }


        // -------- 3. Fetching with some condition ---------
//        TypedQuery<Product> q = em.createQuery("select p from Product p where p.price < 500", Product.class);
//        List<Product> li=q.getResultList();
//        li.forEach(p -> System.out.println(p));


        // --------- 4. Fetching with condition and named parameter --------
//        TypedQuery<Product> q = em.createQuery("select p from Product p where p.price<:P", Product.class);
//        q.setParameter("P",600);
//        List<Product> li = q.getResultList();
//        li.forEach(p -> System.out.println(p));


        // --------- 4. Fetching with condition and named parameter ---------
//        TypedQuery<Product> q=em.createQuery("select p from Product p where p.name like :P", Product.class);
//        q.setParameter("P","L%");
//        List<Product> li=q.getResultList();
//        li.forEach(p-> System.out.println(p));




        // ========== ProductDto ==========

//        TypedQuery<ProductDto> q = em.createQuery("select new com.cg.JPQL.ProductMain.ProductDto(p.name,p.price,p.mfd) from Product p", ProductDto.class);
//        List<ProductDto> li = q.getResultList();
//        for(ProductDto p : li){
//            System.out.println(p);
//        }



        // ========= DML Operations in JPQL ===========


        // 1. -------- Insertion --------
//        em.getTransaction().begin();
//        Product p = new Product();
//        p.setPid(5);
//        p.setName("Window");
//        p.setQty(6);
//        p.setPrice(800);
//        p.setMfd(LocalDate.of(2019,8,11));
//        em.persist(p);
//        em.getTransaction().commit();
//        System.out.println("Inserted...........");


        // 2. -------- Update Price ---------
//        em.getTransaction().begin();
//        Query q = em.createQuery("update Product p set p.price=1200 where p.pid=4");
//        int rows = q.executeUpdate();
//        em.getTransaction().commit();
//        System.out.println(rows + " Updated.....");


        // 3. ------- Delete Product --------
        em.getTransaction().begin();
        Query q=em.createQuery("delete from Product p where p.pid=6");
        int rows=q.executeUpdate();
        em.getTransaction().commit();
        System.out.println(rows+ "Deleted.....");
    }
}
